...
const zeroDetail = { interested: 0, meetingBooked: 0, closed: 0, notInterested: 0, wrongPerson: 0, unsubscribe: 0, neutral: 0, autoReply: 0, outOfOffice: 0 };
...
describe("Reply breakdown: no dummy data when 0 replies", () => {
  ...
  it("should return zero reply classifications when emailsReplied is 0", async () => {
    ...
    mockCallExternalService.mockImplementation((service: any, path: string) => {
      ...
      if (service.url === "http://mock-email" && path.startsWith("/orgs/stats?")) {
        return Promise.resolve({
          transactional: { emailsSent: 10, emailsDelivered: 8, emailsOpened: 3, emailsClicked: 1, emailsReplied: 0, emailsBounced: 0, repliesPositive: 0, repliesNegative: 0, repliesNeutral: 0, repliesAutoReply: 0, repliesDetail: zeroDetail, recipients: 10 },
          broadcast: { emailsSent: 5, emailsDelivered: 4, emailsOpened: 2, emailsClicked: 0, emailsReplied: 0, emailsBounced: 0, repliesPositive: 0, repliesNegative: 0, repliesNeutral: 0, repliesAutoReply: 0, repliesDetail: zeroDetail, recipients: 5 },
        });
      }
      // Lead-service
    ...
    expect(res.body.repliesDetail).toEqual(zeroDetail);
  });

  it("should return reply classifications when emailsReplied > 0", async () => {
    const app = createApp();

    const broadcastDetail = { interested: 1, meetingBooked: 2, closed: 0, notInterested: 1, wrongPerson: 0, unsubscribe: 0, neutral: 0, autoReply: 0, outOfOffice: 1 };
    ...
    mockCallExternalService.mockImplementation((service: any, path: string) => {
      ...
      if (service.url === "http://mock-email" && path.startsWith("/orgs/stats?")) {
        return Promise.resolve({
          transactional: { emailsSent: 10, emailsDelivered: 8, emailsOpened: 3, emailsClicked: 1, emailsReplied: 0, emailsBounced: 0, repliesPositive: 0, repliesNegative: 0, repliesNeutral: 0, repliesAutoReply: 0, repliesDetail: zeroDetail, recipients: 10 },
          broadcast: { emailsSent: 5, emailsDelivered: 4, emailsOpened: 2, emailsClicked: 0, emailsReplied: 5, emailsBounced: 0, repliesPositive: 3, repliesNegative: 1, repliesNeutral: 0, repliesAutoReply: 1, repliesDetail: broadcastDetail, recipients: 5 },
        });
      }
      // Lead-service
      if (service.url === "http://mock-lead" && path.startsWith("/stats?campaignId=")) {
    ...
    expect(res.body.repliesDetail).toEqual(broadcastDetail);
  });
});
