...
describe("Reply breakdown: no dummy data when 0 replies", () => {
  ...
  it("should return zero reply classifications when repliesPositive is 0", async () => {
    const app = createApp();
    ...
    mockCallExternalService.mockImplementation((service: any, path: string) => {
      ...
      if (service.url === "http://mock-email" && path.startsWith("/orgs/stats?")) {
        return Promise.resolve({
          transactional: { emailsSent: 10, emailsDelivered: 8, emailsOpened: 3, emailsClicked: 1, repliesPositive: 0, emailsBounced: 0, repliesWillingToMeet: 0, repliesInterested: 0, repliesNotInterested: 0, repliesOutOfOffice: 0, repliesUnsubscribe: 0, recipients: 10 },
          broadcast: { emailsSent: 5, emailsDelivered: 4, emailsOpened: 2, emailsClicked: 0, repliesPositive: 0, emailsBounced: 0, repliesWillingToMeet: 0, repliesInterested: 0, repliesNotInterested: 0, repliesOutOfOffice: 0, repliesUnsubscribe: 0, recipients: 5 },
        });
      }
      // Lead-service
  ...
  it("should return reply classifications when repliesPositive > 0", async () => {
    const app = createApp();
    ...
    mockCallExternalService.mockImplementation((service: any, path: string) => {
      ...
      if (service.url === "http://mock-email" && path.startsWith("/orgs/stats?")) {
        return Promise.resolve({
          transactional: { emailsSent: 10, emailsDelivered: 8, emailsOpened: 3, emailsClicked: 1, repliesPositive: 0, emailsBounced: 0, repliesWillingToMeet: 0, repliesInterested: 0, repliesNotInterested: 0, repliesOutOfOffice: 0, repliesUnsubscribe: 0, recipients: 10 },
          broadcast: { emailsSent: 5, emailsDelivered: 4, emailsOpened: 2, emailsClicked: 0, repliesPositive: 5, emailsBounced: 0, repliesWillingToMeet: 2, repliesInterested: 1, repliesNotInterested: 1, repliesOutOfOffice: 1, repliesUnsubscribe: 0, recipients: 5 },
        });
      }
      // Lead-service
      if (service.url === "http://mock-lead" && path.startsWith("/stats?campaignId=")) {
    ...
    expect(res.status).toBe(200);
    expect(res.body.repliesPositive).toBe(5);
