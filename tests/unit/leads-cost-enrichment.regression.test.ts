 ...
 * 2. Read delivery status fields directly from each lead (PR #171)
 * 3. Batch-fetch enrichmentRun costs and attach them to each lead
 *
 * Lead-service returns: { id, email, servedAt, runId, contacted, delivered, bounced, replied, replyClassification, enrichment: { firstName, ... } }
...
describe("leads endpoint cost enrichment", () => {
  it("should flatten enrichment and attach enrichmentRun to leads", () => {
    ...
    // Step 2: Attach run data (mirrors leads.ts logic)
    ...
    // Lead with null enrichment should have null fields and status "served"
