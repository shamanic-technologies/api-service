...
// ── Brand grain ──────────────────────────────────────────────────────────────
// The offer-grain reads above, one level up: a brand runs SEVERAL acquisition
// channels at once, and features-service answers across all of them with the
// per-channel breakdown on the same body. It exists because the brand Overview
// read ONE channel's money and paired it with billing's BRAND-wide daily budget —
// a fraction with two grains in it, both halves real, about different things.
// Query objects below are PASSTHROUGH documentation of the params known today,
// never a whitelist — the gateway forwards the caller's query string verbatim
// (CLAUDE.md #11) — and the response is downstream-owned and passed through
// unchanged, per-channel breakdown included (CLAUDE.md #8).
...
registry.registerPath({
  ...
  description:
    "A brand's per-audience economics, across every acquisition channel it runs, with the channel set on the same body. " +
...
registry.registerPath({
  ...
  description:
    "A brand's per-day activity, across every acquisition channel it runs, day buckets merged. " +
  ...
  request: {
    ...
    query: z.object({
      timezone: z.string().openapi({ example: "America/New_York" }).describe("IANA timezone used for calendar day ordering (required)"),
      days: z.string().optional().openapi({ example: "7" }).describe("Number of days to return (features-service owns the default)"),
      pricing: z.string().optional().openapi({ example: "net" }).describe("Pricing basis for the forecast's cost divisor: gross (default) | net. Owned and validated by features-service"),
    }).passthrough(),
  },
  responses: {
